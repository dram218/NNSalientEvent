...
string Labeler::getRealLabel(const string rawLabel){
  ...
  string realLabel = firstWord + "%%%" + secondWord;
  return realLabel;
}
...
double Labeler::predict(const vector<Feature>& features, string& output) {
  vector<double> labelprobs;
  int label = m_classifier.predict(features, labelprobs);
