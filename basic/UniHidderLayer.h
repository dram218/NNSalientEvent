...
public:
  ...
  inline void initial(Tensor<xpu, 2, double> W, Tensor<xpu, 2, double> b, bool bUseB = true, int funcType = 0) {
...
public:
  inline void ComputeForwardScore(Tensor<xpu, 2, double> x, Tensor<xpu, 2, double> y) {
    y = dot(x, _W.T());
    if (_bUseB)
      y = y + _b;
    if (_funcType == 0)
  ...
  inline void ComputeBackwardLoss(Tensor<xpu, 2, double> x, Tensor<xpu, 2, double> y, Tensor<xpu, 2, double> ly, Tensor<xpu, 2, double> lx) {
    //_gradW
