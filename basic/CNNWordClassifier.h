...
public:
  ...
  inline void init(const NRMat<double>& wordEmb, int wordcontext, int labelSize, int wordHiddenSize, int hiddenSize) {
    ...
    _wordGatedPooling.initial(_wordHiddenSize, _wordHiddenSize, true, 40, 3);
    _wordTanhProject.initial(_hiddenSize, _poolmanners * _wordHiddenSize, true, 50, 0);
  ...
  inline double process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      //word related cache
      Tensor<xpu, 3, double> wordprime[sentNum], wordprimeLoss[sentNum], wordprimeMask[sentNum];
      Tensor<xpu, 3, double> wordrepresent[sentNum], wordrepresentLoss[sentNum];
      Tensor<xpu, 4, double> wordinputcontext[sentNum], wordinputcontextLoss[sentNum];
      Tensor<xpu, 3, double> wordInput[sentNum], wordInputLoss[sentNum];
      Tensor<xpu, 3, double> wordHidden[sentNum], wordHiddenLoss[sentNum], wordHiddenLossTmp[sentNum];
      vector<vector<Tensor<xpu, 2, double> > > wordPool(sentNum), wordPoolLoss(sentNum);
      vector<vector<Tensor<xpu, 3, double> > > wordPoolIndex(sentNum), wordPoolIndexLoss(sentNum);

      Tensor<xpu, 3, double> wordGateweight[sentNum], wordGateweightLoss[sentNum], wordGateweightIndex[sentNum];
      Tensor<xpu, 2, double> wordGateweightsum[sentNum], wordGateweightsumLoss[sentNum];

      Tensor<xpu, 2, double> wordPoolmerge[sentNum], wordPoolmergeLoss[sentNum];
      Tensor<xpu, 2, double> wordProject[sentNum], wordProjectLoss[sentNum];
      Tensor<xpu, 2, double> wordOutput[sentNum], wordOutputLoss[sentNum], wordScores[sentNum];
      ...
      for(int idx = 0; idx < sentNum; idx++){
        ...
        vector<Tensor<xpu, 3, double> > tempWordPoolIndexLoss(_poolmanners);
        for (int idm = 0; idm < _poolmanners; idm++) {
          tempWordPool[idm] = NewTensor<xpu>(Shape2(1, wordHiddenSize), 0.0);
          tempWordPoolLoss[idm] = NewTensor<xpu>(Shape2(1, wordHiddenSize), 0.0);
          tempWordPoolIndex[idm] = NewTensor<xpu>(Shape3(word_num, 1, wordHiddenSize), 0.0);
          tempWordPoolIndexLoss[idm] = NewTensor<xpu>(Shape3(word_num, 1, wordHiddenSize), 0.0);
        }
        ...
        wordPoolmerge[idx] = NewTensor<xpu>(Shape2(1, _poolmanners * _wordHiddenSize), 0.0);
        wordPoolmergeLoss[idx] = NewTensor<xpu>(Shape2(1, _poolmanners * _wordHiddenSize), 0.0);
        wordProject[idx] = NewTensor<xpu>(Shape2(1, _hiddenSize), 0.0);
        wordProjectLoss[idx] = NewTensor<xpu>(Shape2(1, _hiddenSize), 0.0);
        wordOutput[idx] = NewTensor<xpu>(Shape2(1, _labelSize), 0.0);
        wordOutputLoss[idx] = NewTensor<xpu>(Shape2(1, _labelSize), 0.0);
      ...
      for(int idx = 0; idx < sentNum; idx++){
        ...
        //word convolution
        for (int idy = 0; idy < word_num; idy++)
          _wordCnnProject.ComputeForwardScore(wordInput[idx][idy], wordHidden[idx][idy]);

        //word pooling
        if ((_remove > 0 && _remove != 1) || (_remove < 0 && _remove == -1) || _remove == 0) {
          avgpool_forward(wordHidden[idx], wordPool[idx][0], wordPoolIndex[idx][0]);
        }
        if ((_remove > 0 && _remove != 2) || (_remove < 0 && _remove == -2) || _remove == 0) {
          maxpool_forward(wordHidden[idx], wordPool[idx][1], wordPoolIndex[idx][1]);
        }
        if ((_remove > 0 && _remove != 3) || (_remove < 0 && _remove == -3) || _remove == 0) {
          minpool_forward(wordHidden[idx], wordPool[idx][2], wordPoolIndex[idx][2]);
        }

        //gated pooling
        if ((_remove > 0 && _remove != 4) || (_remove < 0 && _remove == -4) || _remove == 0) {
          for (int idy = 0; idy < word_num; idy++) {
            _wordGatedPooling.ComputeForwardScore(wordHidden[idx][idy], wordGateweight[idx][idy]);
          }
          sumpool_forward(wordGateweight[idx], wordGateweightsum[idx], wordGateweightIndex[idx]);
          for (int idy = 0; idy < word_num; idy++) {
            wordPoolIndex[idx][3][idy] = wordGateweight[idx][idy] / wordGateweightsum[idx];
          }
          for (int idy = 0; idy < word_num; idy++) {
            wordPool[idx][3] += wordHidden[idx][idy] * wordPoolIndex[idx][3][idy];
          }
        }

        concat(wordPool[idx], wordPoolmerge[idx]);
        _wordTanhProject.ComputeForwardScore(wordPoolmerge[idx], wordProject[idx]);
        _olayer_linear.ComputeForwardScore(wordProject[idx], wordOutput[idx]);
      }


      int optLabel = -1;
      for (int i = 0; i < _labelSize; ++i) {
        if (optLabel < 0 || wordOutput[0][0][i] > wordOutput[0][0][optLabel])
          optLabel = i;
      }
...
//        if (example.m_labels[i] >= 0) {
//          wordOutputLoss[0][0][i] = (wordScores[0][0][i] / sum2 - example.m_labels[i]) / example_num;
//        }
//      }
      ...
      for(int idx = 0; idx < sentNum; idx++){
        _olayer_linear.ComputeBackwardLoss(wordProject[idx], wordOutput[idx], wordOutputLoss[idx], wordProjectLoss[idx]);
        _wordTanhProject.ComputeBackwardLoss(wordPoolmerge[idx], wordProject[idx], wordProjectLoss[idx], wordPoolmergeLoss[idx]);
        ...
        if ((_remove > 0 && _remove != 1) || (_remove < 0 && _remove == -1) || _remove == 0) {
          ...
          pool_backward(wordPoolLoss[idx][0], wordPoolIndex[idx][0], wordHiddenLossTmp[idx]);
          wordHiddenLoss[idx] = wordHiddenLoss[idx] + wordHiddenLossTmp[idx];
        }
        if ((_remove > 0 && _remove != 2) || (_remove < 0 && _remove == -2) || _remove == 0) {
          wordHiddenLossTmp[idx] = 0.0;
          pool_backward(wordPoolLoss[idx][1], wordPoolIndex[idx][1], wordHiddenLossTmp[idx]);
          wordHiddenLoss[idx] = wordHiddenLoss[idx] + wordHiddenLossTmp[idx];
        }
        if ((_remove > 0 && _remove != 3) || (_remove < 0 && _remove == -3) || _remove == 0) {
          wordHiddenLossTmp[idx] = 0.0;
          pool_backward(wordPoolLoss[idx][2], wordPoolIndex[idx][2], wordHiddenLossTmp[idx]);
          wordHiddenLoss[idx] = wordHiddenLoss[idx] + wordHiddenLossTmp[idx];
        }
        ...
        if ((_remove > 0 && _remove != 4) || (_remove < 0 && _remove == -4) || _remove == 0) {
          for (int idy = 0; idy < word_num; idy++) {
            wordPoolIndexLoss[idx][3][idy] = wordPoolLoss[idx][3] * wordHidden[idx][idy];
            wordHiddenLoss[idx][idy] += wordPoolLoss[idx][3] * wordPoolIndex[idx][3][idy];
          }

          for (int idy = 0; idy < word_num; idy++) {
            wordGateweightLoss[idx][idy] += wordPoolIndexLoss[idx][3][idy] / wordGateweightsum[idx];
            wordGateweightsumLoss[idx] -= wordPoolIndexLoss[idx][3][idy] * wordGateweight[idx][idy] / wordGateweightsum[idx] / wordGateweightsum[idx];
          }
          ...
          wordHiddenLossTmp[idx] = 0.0;
          for (int idy = 0; idy < word_num; idy++) {
            _wordGatedPooling.ComputeBackwardLoss(wordHidden[idx][idy], wordGateweight[idx][idy], wordGateweightLoss[idx][idy], wordHiddenLossTmp[idx][idy]);
        ...
        //word convolution
        for (int idy = 0; idy < word_num; idy++) {
          _wordCnnProject.ComputeBackwardLoss(wordInput[idx][idy], wordHidden[idx][idy], wordHiddenLoss[idx][idy], wordInputLoss[idx][idy]);
        }
  ...
  int predict(const vector<Feature>& features, vector<double>& results) {
    ...
    Tensor<xpu, 3, double> wordGateweight[sentNum], wordGateweightIndex[sentNum];
...
//    }
  ...
  double computeScore(const Example& example) {
    ...
    //input setting, and linear setting
    {
      ...
      if ((_remove > 0 && _remove != 4) || (_remove < 0 && _remove == -4) || _remove == 0) {
        for (int idy = 0; idy < word_num; idy++) {
          _wordGatedPooling.ComputeForwardScore(hidden[idy], gateweight[idy]);
        }
  ...
  void checkgrads(const vector<Example>& examples, int iter) {
    ...
    checkgrad(examples, _wordGatedPooling._W, _wordGatedPooling._gradW, "_gated_pooling._W", iter);
    checkgrad(examples, _wordGatedPooling._b, _wordGatedPooling._gradb, "_gated_pooling._b", iter);

    checkgrad(examples, _wordTanhProject._W, _wordTanhProject._gradW, "_tanh_project._W", iter);
    checkgrad(examples, _wordTanhProject._b, _wordTanhProject._gradb, "_tanh_project._b", iter);

    checkgrad(examples, _wordCnnProject._W, _wordCnnProject._gradW, "_cnn_project._W", iter);
    checkgrad(examples, _wordCnnProject._b, _wordCnnProject._gradb, "_cnn_project._b", iter);
